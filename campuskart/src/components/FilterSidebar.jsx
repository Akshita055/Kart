export function FilterSidebar({ selectedCategory, setSelectedCategory, maxPrice, setMaxPrice }) {
  ...
  return (
    <aside className='sticky top-24 hidden h-fit space-y-6 rounded-3xl border border-white/40 bg-white/70 p-5 backdrop-blur-xl lg:block dark:border-slate-700/40 dark:bg-slate-900/70'>
      ...
      <div className='space-y-2'>
        <h3 className='text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400'>Listing Type</h3>
        <p className='rounded-2xl bg-slate-100 px-3 py-2 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300'>
          Includes products, notes marketplace, and skill exchange listings.
        </p>
      </div>

      <div className='space-y-2'>
        <h3 className='text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400'>Nearby Radius</h3>
        <p className='rounded-2xl bg-slate-100 px-3 py-2 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300'>
          Home feed auto-uses your location and shows nearest active listings first.
        </p>
      </div>
    </aside>
  )
